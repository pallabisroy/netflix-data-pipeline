...
# Snowflake connection — uses Streamlit's built-in connector
# Handles token refresh, retries, and connection pooling automatically
...
# ============================================
    ...
    with col2:
        korea = run_query("""
            SELECT TRIM(c.value) AS country, COUNT(*) AS titles
            FROM NETFLIX_CLEAN,
                LATERAL FLATTEN(input => SPLIT(country, ',')) c
            WHERE TRIM(c.value) IN ('South Korea', 'Japan', 'India', 'United Kingdom', 'United States')
